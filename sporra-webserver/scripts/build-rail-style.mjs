...
//      theirs to choose; ours all carry the `sporra-` prefix so a future
...
const USER_AGENT =
  'Sporra/0.1 (+https://github.com/zhekch/sporra; build-rail-style.mjs)';
...
async function main() {
  ...
  for (const layer of upstream.layers) {
    ...
    out.metadata = { ...(out.metadata ?? {}), 'sporra:group': group.key };
    kept.push(out);
  }
  ...
  for (const layer of kept) {
    const group = layer.metadata['sporra:group'];
    const seen = [];
    const walk = (v) => {
    ...
    // Longest prefix first: every `sporra-orm-sdf:` also starts with
    // `sporra-orm`, and crediting the wrong sprite would load the wrong 1.5 MB.
    for (const s of seen) {
  ...
  const payload = {
    ...
    groups: GROUPS.map(({ key, label }) => ({
      key,
      label,
      layers: kept.filter((l) => l.metadata['sporra:group'] === key).map((l) => l.id),
    })),
  };
