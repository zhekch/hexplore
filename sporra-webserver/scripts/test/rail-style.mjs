...
{
  const bad = style.layers.filter((l) => !l.id.startsWith('sporra-orm-'));
  check(!bad.length, 'every layer id is namespaced', bad.slice(0, 3).map((l) => l.id).join(', '));
  const badSrc = Object.keys(style.sources).filter((s) => !s.startsWith('sporra-orm-'));
...
console.log('\nNo source is asked for a zoom it has no data at');
{
  ...
  const range = (name) => {
    const s = style.sources[`sporra-orm-${name}`];
  ...
  check(style.sources['sporra-orm-openrailwaymap_standard']?.minzoom === 8, 'and the standard composite starts at z8');
}
...
console.log('\nEvery URL handed to MapLibre is absolute');
{
  ...
  for (const [id, src] of Object.entries(style.sources)) {
    ...
    check(/^https?:\/\//.test(abs), `${id.replace('sporra-orm-', '')} becomes absolute`, abs);
  }
  ...
  const tpl = railUrl(style.sources['sporra-orm-high'].tiles[0], origin);
  ...
  check(
    new URL(style.sources['sporra-orm-high'].tiles[0], origin).href.includes('%7B'),
...
console.log('\nWhat a clicked feature says');
{
  ...
  const spiez = describeRailFeature({
    source: 'sporra-orm-openrailwaymap_standard',
    sourceLayer: 'standard_railway_platforms',
    id: 'relation-9068328',
    geometry: { type: 'Polygon' },
...
console.log('\nThe technical infrastructure is a filter, not a group');
{
  ...
  check(reads.includes('sporraTechnical'), 'the switch is a global-state key', reads.slice(0, 60));
...
console.log('\nAnd it is put through MapLibre\'s own parser, not eyeballed');
{
  ...
  if (featureFilter) {
    ...
    const track = technicalFilter('railway_line_high');
    const station = technicalFilter('standard_railway_text_stations');
    const off = { sporraTechnical: false };
    const on = { sporraTechnical: true };
