...
const dir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
console.log('\nWhat reaches their server');
{
  ...
  check(/Sporra/.test(req.ua) && /github/.test(req.ua), 'the User-Agent names the project and links to it', req.ua);
...
console.log('\nThe language is part of what a cached entry is');
{
  ...
  const dir2 = await mkdtemp(path.join(tmpdir(), 'sporra-rail-lang-'));
...
console.log('\nA stale entry is revalidated, not re-downloaded');
{
  ...
  const staleDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
{
  const outDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
{
  const failDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
console.log('\nA burst of distinct tiles is one request each, and no more');
{
  ...
  const burstDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
console.log('\nHow much detail is worth asking for');
{
  ...
  const dDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
console.log('\nA 404 is a failure, not an empty tile');
{
  ...
  const nfDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
{
  const emptyDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
...
{
  const bodyDir = await mkdtemp(path.join(tmpdir(), 'sporra-rail-'));
