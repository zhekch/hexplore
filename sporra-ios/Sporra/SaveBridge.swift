...
/// `window.webkit.messageHandlers.sporraSave`, exactly as it asks
...
final class SaveBridge: NSObject, WKScriptMessageHandlerWithReply {
    ...
    private func save(_ body: [String: Any]) async -> [String: Any] {
        ...
        guard await authorize() else {
            return [
                "ok": false,
                "error": "Sporra cannot add to your photo library. Allow it in Settings to save the picture.",
            ]
        }
