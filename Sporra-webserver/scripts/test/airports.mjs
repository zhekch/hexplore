...
console.log('\nThe layers cover the groups, and their filters compile');
{
  ...
  check(layers.every((l) => l.id.startsWith('sporra-air-')), 'every layer id is namespaced');
...
console.log('\nThe important airports are installed last, and so place first');
{
  ...
  check(added.at(-1) === 'sporra-air-large', 'so the large airports end up top-most', String(added.at(-1)));
  check(declared[0] === 'sporra-air-large', 'and are declared first, which is what makes them most important');
