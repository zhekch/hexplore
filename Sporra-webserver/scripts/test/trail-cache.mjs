...
const dir = await mkdtemp(path.join(tmpdir(), 'sporra-trails-'));
...
console.log('\nWhat reaches their server');
{
  ...
  check(/Sporra/.test(req.ua) && /github/.test(req.ua),
...
console.log('\nRevalidation, and what a 304 costs');
{
  ...
  const d2 = await mkdtemp(path.join(tmpdir(), 'sporra-trails-rv-'));
...
{
  const d3 = await mkdtemp(path.join(tmpdir(), 'sporra-trails-down-'));
...
{
  const d4 = await mkdtemp(path.join(tmpdir(), 'sporra-trails-stale-'));
...
console.log('\nNothing is fetched that a person did not just look at');
{
  ...
  const d5 = await mkdtemp(path.join(tmpdir(), 'sporra-trails-cold-'));
