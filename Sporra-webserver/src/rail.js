...
// The `sporra-orm-` namespace every id in the overlay carries is baked into
...
const TECHNICAL_STATE = 'sporraTechnical';
...
export function installRail(map, { font, theme, before, groups, technical = false, detail = {}, lang = null, fastAdd = true }) {
  ...
  return addLayers(map, STYLE.layers.map((layer) => {
    const on = railGroupOn(groups, layer.metadata['sporra:group']);
...
export function describeRailFeature(feature) {
  ...
  return {
    ...
    source: feature.source ? String(feature.source).replace(/^sporra-orm-/, '') : null,
    sourceLayer,
