...
/// Whether the address you typed is a Hexplore server, and whether it is up.
///
/// The Settings screen used to answer that only by implication: you typed an
/// address, looked at the map, and either the site appeared or a web view sat
...
/// something. So the check is the body: `{"app": "hexplore"}`, and anything else
...
final class ServerCheck: ObservableObject {
    ...
    enum State: Equatable {
        ...
        case unset
        case checking
        /// A Hexplore server, and the build it is running.
    ...
    private static func ask(_ url: URL, using session: URLSession) async -> State {
        do {
            ...
            guard (json?["app"] as? String) == "hexplore" else { return .notHexplore }
...
extension ServerCheck.State {
    ...
    var label: String {
        ...
        case .notHexplore: return "Received invalid response"
    ...
    var symbol: String? {
        ...
        case .notHexplore: return "exclamationmark.triangle.fill"
